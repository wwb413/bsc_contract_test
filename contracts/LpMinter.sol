...
contract LpMinter is Ownable {
    ...
    function saferewardTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > rewardBal) {
            reward.transferWithLock(_to, rewardBal);
        } else {
            reward.transferWithLock(_to, _amount);
        }
    }
    ...
    // Setup rewardder Token Reward Release 
    function setrewardRewardLock(uint256 lock) public onlyOwner {
        reward.setRewardLock(lock);
    }
    
    function setrewardTotalBlockRelease(uint256 totalBlockRelease) public onlyOwner {
        reward.setTotalBlockRelease(totalBlockRelease);
    }
